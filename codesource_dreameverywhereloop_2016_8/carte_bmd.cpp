    ...
    for (int i =0; i<mLocal->mNbr_i; i++)
{
    ...
    for (int i=0; i<mLocal->mNbr_i; i++)
    {
        connect(mCaptureDelegate.at(i), SIGNAL(captureFrameArrived(IDeckLinkVideoInputFrame*, bool, IDeckLinkAudioInputPacket*)), this, SLOT(VideoFrameArrived(IDeckLinkVideoInputFrame*, bool,IDeckLinkAudioInputPacket*)), Qt::QueuedConnection);
...
void carte_bmd::VideoFrameArrived(IDeckLinkVideoInputFrame* _inputFrame, bool _hasNoInputSource, IDeckLinkAudioInputPacket* _audioPacket)
{
   ...
   long int  samplescount = _audioPacket->GetSampleFrameCount();

   emit emitAudioPacket(&audioBytes,samplescount, _identifiant_sender);
...
HRESULT	CaptureDelegate::VideoInputFrameArrived(IDeckLinkVideoInputFrame* inputFrame, IDeckLinkAudioInputPacket* audioPacket)
{
    ...
    emit captureFrameArrived(inputFrame, hasNoInputSource, audioPacket);
